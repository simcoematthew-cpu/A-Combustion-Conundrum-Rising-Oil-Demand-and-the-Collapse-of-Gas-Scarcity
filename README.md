...
Quantifies the supply concentration risk — tracking the Permian's growing share of U.S. shale gas production over time
Proves price insensitivity — demonstrating that ~90% of Permian drilling revenue comes from oil, not gas
Models the consumer impact — predicting gas prices would spike 100-140% if Permian supply were removed
Documents the "uncorking" effect — showing how falling oil prices reduce pipeline congestion as associated gas production declines

The analysis combines EIA production data from three major shale basins (Permian, Appalachia, Haynesville), commodity price data (WTI crude, Henry Hub natural gas), and regional storage metrics to build a counterfactual simulation: what would gas prices look like without the Permian?
